...
function CTip({active,payload,label}){
  ...
  return(
    <div style={{background:"#152535",border:"1px solid #1e293b",borderRadius:10,padding:"10px 13px",fontSize:12,maxWidth:280,boxShadow:"0 8px 32px rgba(0,0,0,.6)"}}>
...
function Metric({title,value,sub,color="#4a7c8e",large}){
  return(
    <div style={{background:"#152535",borderRadius:12,padding:large?"18px 20px":"13px 15px",border:"1px solid #1e293b",position:"relative",overflow:"hidden"}}>
  ...
  return(
    <div>
      <label onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)}
        ...
        style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"16px",border:`2px dashed ${drag?"#4a7c8e":fileName?"#1e4d2b":"#1e3448"}`,borderRadius:12,cursor:"pointer",background:drag?"#0c1a2e":fileName?"#020d06":"#0d1b2a",transition:"all .2s",textAlign:"center"}}>
        <div style={{width:34,height:34,borderRadius:8,background:"#152535",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid #1e293b"}}>{icon}</div>
        <div>
          <div style={{fontWeight:600,color:fileName?"#22c55e":"#cbd5e1",fontSize:13,marginBottom:2}}>{fileName||label}</div>
          <div style={{fontSize:11,color:"#2a4560"}}>{fileName?"Loaded — click to replace":"Drag & drop or click"}</div>
        </div>
        <input type="file" accept={accept} style={{display:"none"}} onChange={e=>e.target.files?.[0]&&onFile(e.target.files[0])}/>
      </label>
      {hint&&<div style={{fontSize:11,color:"#2a4560",marginTop:3}}>{hint}</div>}
    </div>
  );
}
...
function ZoomBar({isZoomed,onReset}){
  return isZoomed
    ?<button onClick={onReset} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 9px",background:"#1e3448",border:"1px solid #334155",borderRadius:6,color:"#94a3b8",cursor:"pointer",fontSize:11}}><ZoomOut size={10}/> Reset zoom</button>
    :<span style={{fontSize:11,color:"#2a4560"}}>Drag on chart to zoom</span>;
}

function Empty({msg}){
  return(<div style={{minHeight:160,display:"flex",alignItems:"center",justifyContent:"center",border:"1px dashed #1e293b",borderRadius:12}}><div style={{fontSize:13,color:"#2a4560",textAlign:"center",padding:20,maxWidth:300}}>{msg}</div></div>);
}
...
function LinkManager({tasks,sovItems,linksMap,onLinksChange}){
  ...
  return(
    <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:18}}>
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:12}}>
        <input value={taskFilter} onChange={e=>setTaskFilter(e.target.value)} placeholder="Filter tasks…"
          style={{padding:"6px 10px",background:"#0d1b2a",border:"1px solid #1e293b",borderRadius:7,color:"#94a3b8",fontSize:12,outline:"none",width:200}}/>
        <label style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:"#64748b",cursor:"pointer"}}>
          <input type="checkbox" checked={unlinkedOnly} onChange={e=>setUnlinkedOnly(e.target.checked)} style={{accentColor:"#4a7c8e"}}/>
          Unlinked only
        </label>
      ...
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",minWidth:640,borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{background:"#0d1b2a"}}>{["WBS","Task Name","Dates","SOV Scope"].map(h=><th key={h} style={{padding:"7px 10px",textAlign:"left",color:"#475569",fontFamily:"monospace",fontSize:9,borderBottom:"1px solid #1e293b",textTransform:"uppercase",letterSpacing:.5,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
          <tbody>
            {filtered.map((t,i)=>{
              const sid=taskToSov[t.uid];const col=sid?sovColorMap[sid]:null;
              return(
                <tr key={t.uid} style={{borderBottom:"1px solid #1e293b",background:i%2===0?"transparent":"#0d1b2a"}}>
                  <td style={{padding:"7px 10px",fontFamily:"monospace",color:"#4a7c8e",fontSize:10,whiteSpace:"nowrap"}}>{t.id}</td>
                  ...
                  <td style={{padding:"7px 10px",minWidth:240}}>
                    <select value={sid||"none"} onChange={e=>setTaskLink(t.uid,e.target.value)}
                      style={{width:"100%",padding:"5px 9px",background:col?`${col}18`:"#0d1b2a",border:`1px solid ${col||"#1e3448"}`,borderRadius:6,color:col||"#64748b",fontSize:12,cursor:"pointer",outline:"none",fontWeight:col?600:400}}>
                      <option value="none" style={{background:"#0d1b2a",color:"#475569"}}>— Unassigned —</option>
                      {sovItems.map(s=><option key={s.id} value={s.id} style={{background:"#0d1b2a",color:"#cbd5e1"}}>{s.description} ({currency(s.scheduledValue)})</option>)}
                    </select>
                  </td>
                </tr>
              );
            })}
            {!filtered.length&&<tr><td colSpan={4} style={{padding:24,textAlign:"center",color:"#2a4560"}}>No tasks match filter.</td></tr>}
          </tbody>
        </table>
      </div>
      <div style={{marginTop:16,paddingTop:14,borderTop:"1px solid #1e293b"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:7}}>
          {sovItems.map((s,i)=>{const col=PAL[i%PAL.length];const count=(linksMap[s.id]||[]).length;return(<div key={s.id} style={{padding:"8px 10px",borderRadius:8,background:`${col}10`,border:`1px solid ${count>0?col:"#1e3448"}`,display:"flex",justifyContent:"space-between",alignItems:"center",gap:6}}><div style={{overflow:"hidden"}}><div style={{fontSize:11,color:"#cbd5e1",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.description}</div><div style={{fontSize:10,color:col,fontFamily:"monospace"}}>{currency(s.scheduledValue)}</div></div><span style={{flexShrink:0,fontSize:10,fontFamily:"monospace",color:count>0?col:"#2a4560",background:count>0?`${col}20`:"#152535",padding:"1px 7px",borderRadius:99,border:`1px solid ${count>0?`${col}40`:"#1e3448"}`}}>{count}</span></div>);})}
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:18}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <input type="date" value={updateDate} onChange={e=>setUpdateDate(e.target.value)}
            style={{padding:"5px 9px",background:"#0d1b2a",border:"1px solid #1e293b",borderRadius:7,color:"#94a3b8",fontSize:12,outline:"none"}}/>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8,marginBottom:16}}>
        ...
        {baseline&&<Metric title="Baseline at Date" value={currency(baseline)} color="#4a7c8e" sub="planned spend"/>}
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        <thead><tr style={{background:"#0d1b2a"}}>{["Scope","Contract Value","% Complete","Earned Value","Status"].map(h=><th key={h} style={{padding:"7px 10px",textAlign:"left",color:"#475569",fontFamily:"monospace",fontSize:9,borderBottom:"1px solid #1e293b",textTransform:"uppercase",letterSpacing:.5,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
        <tbody>
          {sovItems.map((sov,i)=>{
                ...
                <td style={{padding:"8px 10px",minWidth:160}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <input type="number" min={0} max={100} value={pctVal} onChange={e=>setProgress(sov.id,Number(e.target.value))}
                      style={{width:46,padding:"3px 5px",background:"#0d1b2a",border:`1px solid ${col}`,borderRadius:5,color:col,fontSize:12,fontFamily:"monospace",outline:"none",textAlign:"center"}}/>
                    <span style={{color:"#475569",fontSize:11}}>%</span>
                  </div>
                </td>
                <td style={{padding:"8px 10px",color:col,fontFamily:"monospace",fontWeight:600}}>{currency(earned)}</td>
                <td style={{padding:"8px 10px"}}>{prog?<TrafficLight status={status}/>:<span style={{fontSize:11,color:"#2a4560"}}>—</span>}</td>
              </tr>
            );
          })}
        </tbody>
  ...
  return(
    <div style={{display:"grid",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10}}>
        <Metric title="Contract Value" value={currency(contractTotal)} color="#4a7c8e" large/>
      ...
      <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:20}}>
          ...
          {allDates.length?(
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={sCurveData} margin={{top:8,right:8,left:8,bottom:8}}
                ...
                <ReferenceLine x={todayStr} stroke="#2a4560" strokeDasharray="3 3" label={{value:"Today",fill:"#475569",fontSize:10}}/>
                <Line type="monotone" dataKey="cumTotal" stroke="#4a7c8e" strokeWidth={2} dot={false} strokeDasharray="6 3" name="Baseline (Planned)"/>
                <Line type="monotone" dataKey="actualCum" stroke="#22c55e" strokeWidth={2.5} dot={false} name="Actual Earned"/>
                {combinedZoom.sel?.active&&combinedZoom.sel.start&&combinedZoom.sel.end&&<ReferenceArea x1={combinedZoom.sel.start} x2={combinedZoom.sel.end} fill="#4a7c8e" fillOpacity={0.08}/>}
              </LineChart>
            </ResponsiveContainer>
      ...
      {/* Scope health table */}
      <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:18,overflowX:"auto"}}>
        ...
        <table style={{width:"100%",minWidth:600,borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{background:"#0d1b2a"}}>{["Scope","Contract Value","Baseline % Today","Actual %","Variance","Status"].map(h=><th key={h} style={{padding:"7px 10px",textAlign:"left",color:"#475569",fontFamily:"monospace",fontSize:9,borderBottom:"1px solid #1e293b",textTransform:"uppercase",letterSpacing:.5,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
          <tbody>
            {scopeHealth.map(({sov,pct,baselineToday,diff,status,col},i)=>(
              <tr key={sov.id} style={{borderBottom:"1px solid #1e293b",background:i%2===0?"transparent":"#0d1b2a"}}>
                <td style={{padding:"8px 10px",color:"#cbd5e1",maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                ...
                <td style={{padding:"8px 10px",minWidth:120}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <div style={{flex:1,height:4,background:"#1e3448",borderRadius:2}}><div style={{width:`${Math.min(100,Math.round(pct*100))}%`,height:"100%",background:col,borderRadius:2}}/></div>
...
export default function App(){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0d1b2a",color:"#e2e8f0",fontFamily:"'Inter',system-ui,sans-serif",padding:"24px"}}>
      <style>{`
      ...
      <div style={{maxWidth:1400,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:20,flexWrap:"wrap"}}>
            ...
            <div>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                ...
                <span style={{fontFamily:"monospace",fontSize:9,color:"#8aabb8",letterSpacing:2,textTransform:"uppercase"}}>Scope Spend · Baseline · Progress</span>
              </div>
              <h1 style={{fontFamily:"'Syne',sans-serif",fontSize:26,fontWeight:800,margin:0,lineHeight:1.1,color:"#f8fafc"}}>
                Project Spend<br/><span style={{color:"#8aabb8"}}>Baseline Tracker</span>
              </h1>
            </div>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
            {hasData&&!baselineLocked&&(
              <button onClick={()=>setBaselineLocked(true)}
                style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",background:"#1e3448",border:"1px solid #3b82f6",borderRadius:8,color:"#8aabb8",cursor:"pointer",fontSize:12,fontWeight:600}}>
            ...
            <button onClick={exportReport} disabled={!hasData}
              style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",background:"transparent",border:"1px solid #1e293b",borderRadius:8,color:hasData?"#94a3b8":"#2a4560",cursor:hasData?"pointer":"not-allowed",fontSize:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <DropZone label="Upload MS Project XML" accept=".xml" onFile={handleScheduleFile} fileName={schedFile?`${schedFile} · ${workTasks.length} tasks`:""} icon={<FileText size={15} color="#4a7c8e"/>} hint="File → Save As → XML Format in MS Project"/>
        ...
        {/* SOV info bar */}
        {sovResult&&(
          <div style={{display:"flex",gap:8,alignItems:"center",padding:"8px 12px",background:"#152535",border:"1px solid #1e293b",borderRadius:8,marginBottom:12,fontSize:12,flexWrap:"wrap"}}>
            <Info size={11} color="#4a7c8e"/>
            <span style={{color:"#64748b"}}>SOV:</span>
            <span style={{color:"#f1f5f9",fontFamily:"monospace"}}>{currency(totals.base)}</span><span style={{color:"#2a4560"}}>base +</span>
            <span style={{color:"#f97316",fontFamily:"monospace"}}>{currency(totals.gcgr)}</span><span style={{color:"#2a4560"}}>Gen Req +</span>
            <span style={{color:"#a855f7",fontFamily:"monospace"}}>{currency(totals.ohp)}</span><span style={{color:"#2a4560"}}>OH&P =</span>
        ...
        {/* Tabs */}
        <div style={{display:"flex",gap:2,marginBottom:14,background:"#152535",padding:3,borderRadius:10,border:"1px solid #1e293b",width:"fit-content",flexWrap:"wrap"}}>
          {TABS.map(t=>(
            <button key={t.id} className="tab-btn" onClick={()=>setActiveTab(t.id)}
              style={{display:"flex",alignItems:"center",gap:5,padding:"6px 12px",borderRadius:7,border:"none",cursor:"pointer",fontSize:12,fontWeight:500,background:activeTab===t.id?"#1e3448":"transparent",color:activeTab===t.id?"#f1f5f9":"#64748b"}}>
              {t.icon}{t.label}
          ...
          <div style={{display:"grid",gap:14}}>
            ...
            {hasData&&(
              <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
                <button onClick={()=>setSelectedScope("all")} style={{padding:"4px 10px",borderRadius:99,border:`1px solid ${selectedScope==="all"?"#4a7c8e":"#1e3448"}`,background:selectedScope==="all"?"#1e3448":"transparent",color:selectedScope==="all"?"#8aabb8":"#64748b",cursor:"pointer",fontSize:12}}>All scopes</button>
                {sovKeys.map((sid,i)=>{const s=byScope[sid];const col=PAL[i%PAL.length];return(<button key={sid} onClick={()=>setSelectedScope(sid)} style={{padding:"4px 10px",borderRadius:99,border:`1px solid ${selectedScope===sid?col:"#1e3448"}`,background:selectedScope===sid?`${col}20`:"transparent",color:selectedScope===sid?col:"#64748b",cursor:"pointer",fontSize:12,maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.sov.description.slice(0,22)}</button>);})}
              </div>
            )}
            ...
            {hasData&&selectedScope!=="all"&&byScope[selectedScope]&&(()=>{
              ...
              return(
                <div style={{display:"grid",gap:12}}>
                  <div style={{background:"#152535",borderRadius:13,border:`1px solid ${col}40`,padding:16,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10}}>
                  ...
                  <div style={{background:"#152535",borderRadius:13,border:"1px solid #1e293b",padding:20}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
                  ...
                  <div style={{background:"#152535",borderRadius:13,border:"1px solid #1e293b",padding:18}}>
                    ...
                    <table style={{width:"100%",minWidth:480,borderCollapse:"collapse",fontSize:12}}>
                      <thead><tr style={{background:"#0d1b2a"}}>{["WBS","Task","Start","Finish","%"].map(h=><th key={h} style={{padding:"6px 9px",textAlign:"left",color:"#475569",fontFamily:"monospace",fontSize:9,borderBottom:"1px solid #1e293b",textTransform:"uppercase",letterSpacing:.5}}>{h}</th>)}</tr></thead>
                      <tbody>{lt.map((t,i)=>(<tr key={i} style={{borderBottom:"1px solid #0f172a"}}><td style={{padding:"6px 9px",fontFamily:"monospace",color:"#4a7c8e",fontSize:10}}>{t.id}</td><td style={{padding:"6px 9px",color:"#cbd5e1"}}>{t.name}</td><td style={{padding:"6px 9px",fontFamily:"monospace",color:"#64748b",fontSize:10}}>{fmtDate(t.startDate)}</td><td style={{padding:"6px 9px",fontFamily:"monospace",color:"#64748b",fontSize:10}}>{fmtDate(t.finishDate)}</td><td style={{padding:"6px 9px",minWidth:80}}><div style={{display:"flex",alignItems:"center",gap:5}}><div style={{flex:1,height:3,background:"#1e3448",borderRadius:2}}><div style={{width:`${Math.min(100,Math.round(t.percentComplete*100))}%`,height:"100%",background:col,borderRadius:2}}/></div><span style={{fontFamily:"monospace",fontSize:9,color:"#64748b",minWidth:22}}>{Math.round(t.percentComplete*100)}%</span></div></td></tr>))}</tbody>
                    </table>
                  </div>
                </div>
              );
          ...
          <div style={{display:"grid",gap:14}}>
            ...
            {hasData&&(
              <>
                    ...
                    <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:20}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
                        <div>
                          ...
                          <p style={{color:"#475569",fontSize:12,margin:0}}>Each scope's bell curve across its own schedule dates · white line = project monthly total</p>
                        </div>
                        {c1Zoomed?<button onClick={()=>{setC1Left(0);setC1Right(null);}} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 9px",background:"#1e3448",border:"1px solid #334155",borderRadius:6,color:"#94a3b8",cursor:"pointer",fontSize:11}}><ZoomOut size={10}/> Reset zoom</button>:<span style={{fontSize:11,color:"#2a4560"}}>Drag to zoom</span>}
                      </div>
                      <div style={{height:460,cursor:"crosshair"}}>
                        <ResponsiveContainer width="100%" height="100%">
                          <ComposedChart data={c1Sliced} margin={{top:16,right:20,left:8,bottom:8}}
                            ...
                            onMouseUp={()=>{
                              if(c1Sel?.active&&c1Sel.start&&c1Sel.end&&c1Sel.start!==c1Sel.end){
                                const a=monthlyData.findIndex(d=>d.month===c1Sel.start),b=monthlyData.findIndex(d=>d.month===c1Sel.end);
                                if(a>=0&&b>=0){setC1Left(Math.min(a,b));setC1Right(Math.max(a,b));}
                              }
                              setC1Sel(null);
                            }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#1e3448"/>
                            ...
                            <Tooltip content={({active,payload,label})=>{
                              ...
                              return(<div style={{background:"#152535",border:"1px solid #1e293b",borderRadius:10,padding:"10px 13px",fontSize:12,maxWidth:300,boxShadow:"0 8px 32px rgba(0,0,0,.6)"}}>
                            ...
                            <Line type="monotone" dataKey="total" stroke="#ffffff" strokeWidth={3} dot={{fill:"#ffffff",r:3,strokeWidth:0}} activeDot={{r:6,fill:"#ffffff",stroke:"#4a7c8e",strokeWidth:2}} name="▸ Project Total" strokeOpacity={1} legendType="line"/>
                            {c1Sel?.active&&c1Sel.start&&c1Sel.end&&<ReferenceArea x1={c1Sel.start} x2={c1Sel.end} fill="#4a7c8e" fillOpacity={0.08}/>}
                          </ComposedChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={monthlyData} margin={{top:16,right:20,left:8,bottom:8}}>
                        ...
                        <Tooltip content={({active,payload,label})=>{
                          if(!active||!payload?.length)return null;
                          const totalVal=monthlyData.find(d=>d.month===label)?.total||0;
                          return(<div style={{background:"#152535",border:"1px solid #1e293b",borderRadius:10,padding:"10px 13px",boxShadow:"0 8px 32px rgba(0,0,0,.6)"}}>
                            <div style={{color:"#64748b",marginBottom:5,fontFamily:"monospace",fontSize:10}}>{label}</div>
                        ...
                        <Area type="monotone" dataKey="total" stroke="#ffffff" strokeWidth={3} fill="url(#totMonthGrad)" name="Project Total" dot={{fill:"#ffffff",r:4,strokeWidth:0}} activeDot={{r:6,fill:"#ffffff",stroke:"#4a7c8e",strokeWidth:2}}/>
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                  ...
                  return(
                    <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:20}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
                        ...
                        {isZoomed&&<button onClick={()=>{setMLeft(0);setMRight(null);}} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 9px",background:"#1e3448",border:"1px solid #334155",borderRadius:6,color:"#94a3b8",cursor:"pointer",fontSize:11}}><ZoomOut size={10}/> Reset zoom</button>}
                        {!isZoomed&&<span style={{fontSize:11,color:"#2a4560"}}>Drag to zoom</span>}
                      </div>
                      <div style={{height:480,cursor:"crosshair"}}>
                        <ResponsiveContainer width="100%" height="100%">
                          ...
                          return(<Line key={sid} type="monotone" dataKey={d=>d.month>=psKey&&d.month<=peKey?(d[key]||0):null} stroke={col} strokeWidth={2.5} dot={false} name={key} connectNulls={false} activeDot={{r:5,fill:col,stroke:"#fff",strokeWidth:1}}/>);
                            ...
                            {mSel?.active&&mSel.start&&mSel.end&&<ReferenceArea x1={mSel.start} x2={mSel.end} fill="#4a7c8e" fillOpacity={0.08}/>}
                          </ComposedChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                ...
                <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:20,overflowX:"auto"}}>
                  ...
                  <table style={{width:"100%",minWidth:500,borderCollapse:"collapse",fontSize:12}}>
                    <thead>
                      <tr style={{background:"#0d1b2a"}}>
                        ...
                        <th style={{padding:"7px 12px",textAlign:"right",color:"#4a7c8e",fontFamily:"monospace",fontSize:9,borderBottom:"1px solid #1e293b",textTransform:"uppercase",letterSpacing:.5}}>Cumulative</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(()=>{
                        ...
                        return monthlyData.map((row,i)=>{
                          ...
                          return(
                            <tr key={row.month} style={{borderBottom:"1px solid #1e293b",background:i%2===0?"transparent":"#0d1b2a"}}>
                              ...
                              {sovKeys.map(sid=>{
                                ...
                                return(<td key={sid} style={{padding:"7px 10px",textAlign:"right",fontFamily:"monospace",color:row[key]>0?col:"#2a4560",fontSize:11}}>{row[key]>0?currency(row[key]):"—"}</td>);
                              })}
                              ...
                              <td style={{padding:"7px 12px",textAlign:"right",fontFamily:"monospace",color:"#4a7c8e",fontSize:11}}>{currency(runningTotal)}</td>
                            </tr>
                          );
                        });
                      })()}
                    ...
                    <tfoot>
                      <tr style={{borderTop:"2px solid #1e293b"}}>
                        ...
                        <td style={{padding:"8px 12px",textAlign:"right",fontFamily:"monospace",color:"#4a7c8e",fontWeight:700,fontSize:13}}>{currency(monthlyData.reduce((s,r)=>s+r.total,0))}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
        ...
        {activeTab==="links"&&(
          workTasks.length&&sovItems.length
            ...
            :<div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:40,textAlign:"center"}}><Link2 size={24} color="#1e3448" style={{marginBottom:8}}/><div style={{fontSize:13,color:"#2a4560"}}>Upload both files to use the link manager.</div></div>
        )}
          ...
          <div style={{background:"#152535",borderRadius:14,border:"1px solid #1e293b",padding:18,overflowX:"auto"}}>
            ...
            {sovItems.length?(
              <table style={{width:"100%",minWidth:660,borderCollapse:"collapse",fontSize:12}}>
                <thead><tr style={{background:"#0d1b2a"}}>{["Ph","Description","Base","+ Gen Req","+ OH&P","Contract Value","Tasks"].map(h=><th key={h} style={{padding:"7px 9px",textAlign:"left",color:"#475569",fontFamily:"monospace",fontSize:9,borderBottom:"1px solid #1e293b",textTransform:"uppercase",letterSpacing:.5,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
                <tbody>
                  {sovItems.map((s,i)=>{const col=PAL[i%PAL.length];const count=(linksMap[s.id]||[]).length;return(<tr key={s.id} style={{borderBottom:"1px solid #1e293b",background:i%2===0?"transparent":"#0d1b2a"}}><td style={{padding:"7px 9px",fontFamily:"monospace",color:"#475569",fontSize:10}}>{s.phase||"—"}</td><td style={{padding:"7px 9px",color:"#cbd5e1",maxWidth:170,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.description}</td><td style={{padding:"7px 9px",color:"#94a3b8",fontFamily:"monospace"}}>{currency(s.baseValue)}</td><td style={{padding:"7px 9px",color:"#f97316",fontFamily:"monospace",fontSize:10}}>{currency(s.gcgrShare)}</td><td style={{padding:"7px 9px",color:"#a855f7",fontFamily:"monospace",fontSize:10}}>{currency(s.ohpShare)}</td><td style={{padding:"7px 9px",fontWeight:700,color:col,fontFamily:"monospace"}}>{currency(s.scheduledValue)}</td><td style={{padding:"7px 9px"}}><span style={{fontSize:10,color:count>0?col:"#2a4560",background:count>0?`${col}20`:"#152535",padding:"1px 7px",borderRadius:99,border:`1px solid ${count>0?`${col}40`:"#1e3448"}`}}>{count}</span></td></tr>);})}
                </tbody>
